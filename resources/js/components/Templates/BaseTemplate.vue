<template lang="html">
    <div>
        <rzd-template
            v-if="ui == 'rzd' && dataReady"
            :data="data"
            :alerts="alerts"
        />
        <mrd-dbl-template
            v-if="ui == 'mrd-dbl' && dataReady"
            :data="data"
            :alerts="alerts"
        />
    </div>
...
import MrdDblTemplate from "./MrdDblTemplate.vue";
export default {
    name: "base-template",
    ...
    watch: {
        data: function (newData, oldData) {
            if (
                oldData &&
                newData[newData.length - 1].measured_at !=
                    oldData[oldData.length - 1].measured_at
            ) {
                axios
                    .get("/api/alerts")
                    .then((response) => {
    ...
    methods: {
        ...
        async loadAll() {
            try {
                await Promise.all([this.getLastData(), this.getAlerts()]);
            } catch (e) {
                return e;
            }
            return true;
